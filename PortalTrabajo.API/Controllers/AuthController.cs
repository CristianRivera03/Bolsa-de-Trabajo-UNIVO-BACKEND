...
namespace PortalTrabajo.API.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> SetupInitialAdmin([FromBody] CrearAdminDTO modelo)
        {
            ...
            try
            {
                var adminExistente = await _usuarioRepo.Get(u => u.RolId == 1);
                if (adminExistente != null)
                {
                    rsp.status = false;
                    rsp.msg = "Ya existe un administrador en el sistema.";
        ...
        public async Task<IActionResult> CambiarPassword([FromBody] CambiarPasswordDTO model)
        {
            ...
            try
            {
                ...
                {
                    rsp.status = false;
                    rsp.msg = "Usuario no autorizado.";
                    return Unauthorized(rsp);
                }

                await _authService.CambiarPassword(usuarioId, model.CurrentPassword, model.NewPassword);
                
                rsp.status = true;
                rsp.msg = "Contraseña actualizada exitosamente.";
                return Ok(rsp);
            }
            catch (UnauthorizedAccessException ex)
            {
            ...
            {
                rsp.status = false;
                rsp.msg = "Ocurrió un error interno al cambiar la contraseña.";
                return StatusCode(StatusCodes.Status500InternalServerError, rsp);
            }
        }
    }
